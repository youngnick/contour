...
DOCKERIP ?= $(shell docker network inspect kind | jq -r ".[0].IPAM.Config[0].Gateway")
...
.PHONY: make-kind-cluster
make-kind-cluster: ## Make a kind cluster with standard ports forwarded
	./test/scripts/make-kind-cluster.sh

.PHONY: install-contour-working
install-contour-working: | make-kind-cluster ## Install the local working directory version of Contour into a kind cluster
	./test/scripts/install-contour-working.sh

.PHONY: install-contour-release 
install-contour-release: | make-kind-cluster ## Install the release version of Contour in CONTOUR_UPGRADE_FROM_VERSION, defaults to latest
